...
CANCEL_CODE          = "your_code_here"  # code to cancel warning
CAMERA_INDEX         = 0
ENROLL_FRAMES        = 60
CONFIDENCE_THRESHOLD = 70
FACE_DATA_FILE       = "kaneki_owner.yml"
EXE_NAME             = "kaneki_countdown.exe"
# ─────────────────────────────────────────────────────────────
...
def capture_session(cap, label):
    print(f"\n   📸  {label}")
    print("   Press SPACE to start capturing, Q to skip.\n")
    ...
    while True:
        ...
        cv2.imshow("Kaneki Watcher — Enrollment", frame)
...
class WarningWindow:
    def __init__(self, on_cancel, on_timeout):
        ...
        self.root = tk.Tk()
        self.root.title("⚠️ Kaneki Warning")
        ...
        tk.Label(self.root, text="⚠️  UNRECOGNIZED FACE",
                 fg="#ff4444", bg="#1a0000",
                 font=("Courier", 16, "bold")).pack(pady=(20, 4))
        tk.Label(self.root, text="Type your cancel code to abort:",
                 fg="#ffaa00", bg="#1a0000",
                 font=("Courier", 11)).pack()

        self.timer_label = tk.Label(self.root,
                 text=f"Launching in {self._remaining}s…",
                 fg="#ff4444", bg="#1a0000",
    ...
    def _tick(self):
        if self._remaining <= 0:
            ...
            self.on_timeout()
            return
        self.timer_label.config(text=f"Launching in {self._remaining}s…")
        self._remaining -= 1
    ...
    while running:
        ...
        if cmd == "pause":
            if paused:
                ...
                paused = True
                print("⏸️   Watcher PAUSED — Kaneki won't trigger.")
            ...
            if not paused:
                ...
                paused = False
                print("▶️   Watcher RESUMED — back on guard! 👁️\n")
...
def watch(recognizer):
    ...
    print(f"   Triggers after {TRIGGER_DELAY}s → {WARNING_DURATION}s warning → Kaneki launches")
    print(f"   Cancel code: '{CANCEL_CODE}'")
    ...
    while running:
        ...
        if len(faces) == 0:
            ...
            for (x, y, w, h) in faces:
                ...
                if label == 0 and confidence < CONFIDENCE_THRESHOLD:
                    owner_present = True
                    break
            status = "OWNER ✓" if owner_present else "STRANGER ⚠️"
        ...
        if owner_present:
            ...
            if not warning_open:
                ...
                print(f"\r   [{status}] Warning in {remaining:.1f}s…   ",
                      end="", flush=True)
